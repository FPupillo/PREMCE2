...
gammaseq<-seq(0.1,0.33, length.out=lengthparam)
...
model<-simulate_HMM_rev

fit<-fit_HMM_rev

modname<-as.character("HMM_rev")
...
for (j in 1:(lengthparam)){
  #for (b in 1:length(betaseq)){
    ...
    for (n in 1:nrow(Data)){
      ...
      sim<-model(Data=Data, gamma = gammaseq[j], c=cseq[j],
                 initialPs = 0.25)
    ...
    # now fit the data
    est<-fit(data=sim, cBound = c(0.1,1), gammaBound = c(0.1, 0.33),
             initialPs =0.25)
    ...
    #append the data
    temp[nrow(temp)+1, ]<-c(cseq[j], estC, gammaseq[j], estGamma,
                        est$BIC)
