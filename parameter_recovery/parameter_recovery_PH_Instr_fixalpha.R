#------------------------------------------------------------------------------#
# simulating trials switch points and character - Pearce Hall -Instr
...
source("simulation_functions/simulate_PearceHall_Instr.R")
source(("helper_functions/softmax.R"))
source(("helper_functions/chooseMultinom.R"))
source(("helper_functions/BICcompute.R"))
source("likelihood_functions/lik_PearceHall_Instr.R")
source("fitting_functions/fit_PearceHall_Instr_fixalpha.R")
...
names(df)<-c("simBeta", "fitBeta", "simGamma", 
            "fitGamma",  "BIC")

model<-simulate_PearceHall_Instr

fit<-fit_PearceHall_Instr_fixalpha

modname<-as.character("PH_Instr_fixapha")
...
#   for (b in 1:length(betaseq)){
    ...
    for (n in 1:nrow(Data)){
      ...
      sim<-model(Data=Data,alpha=alphaseq[simul], beta =betaseq[simul], 
    ...
    # now fit the data
    est<-fit(data=sim, alphaBound = c(0,1),  betaBound = c(1,20), kBound =c(0,1), 
             gammaBound = c(0,1),
             initialV =0.25)
    
    #estAlpha<-est$alphabetaPAR[1]
    
    estBeta<-est$alphabetaPAR[1]
    ...
    #append the data
    temp[nrow(temp)+1, ]<-c( betaseq[simul], 
