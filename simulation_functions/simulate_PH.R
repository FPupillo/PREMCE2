simulate_PH<-function ( Data,alpha = NULL, alpha0, beta, k, gamma, 
                        c = NULL, initialV){
  ...
  categ<-c("Electronic device & accessory","Hand labour tool & accessory" ,
           "Kitchen & utensil", "Outdoor activity & sport item" )
  ...
  for (t in 1: nrow(Data) ){ 
    ...
    # prediction error
    delta <- r -V[Data$respCounter[t]]
    
    # update Q
    V[Data$respCounter[t]]<- V[Data$respCounter[t]]+k*alpha*delta
